...
# Categorieën ojbecten

Er zijn erg veel objectcategorieën. Deze zijn teruggebracht tot 10 (door John)

```{r}
objecten <- read_excel('data/Diefstallen_met_geclusterde_object_waarden.xlsx') %>%
  select(id = `Incident Identificatie`, Object_2)
```
...
diefstal_dummy <- diefstallen %>% 
  ...
  mutate(begin_plus = begin + days(1), 
         ...
         wdag = wday(begin, 
                     ...
                     abbr = F)) %>% 
  filter(zelfde_dag) %>% 
  select(id, buurt, begin_wdag, Object_2) %>%
  dummy() 

# naar csv schrijven
write.csv(diefstal_dummy, 'data/dummyvariabelen.csv', row.names = F)
```
